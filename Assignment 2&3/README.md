...
* First, we played around with the parameters of SVC model to see if we can increase the f1-mean score.
  * We went through C, alpha (different values than the comment), penalty (different values than the comment) and gamma. All of them's best performance were their default values.
